...
def transformPoint(point, connectedPoints, resting): # applies transformations to the point location based on inputted data
    ...
    for line in lineLibrary: # loops through every line
        ...
        #if side(positionTuple, line, normalsLibrary[ln]) == True:
            #passedPoints.append((point, ln))#, side(nextPointPosition, line, normalsLibrary[ln])))
            #passedFuturePoints.append(nextPointPosition)
        if positionTuple[1] > ((positionTuple[0] - line[0]) / (line[2] - line[0])) * (line[3] - line[1]) + line[1]:
            passedPoints.append((point, ln))#, side(nextPointPosition, line, normalsLibrary[ln])))
            passedFuturePoints.append(nextPointPosition)
    ...
    for line in range(len(lineLibrary)):
        ...
        if (point,line) in passedPoints: # or (point,line,1) in passedPoints:
            ...
            mgsintheta = (gravity[1] + newTransformVector[1]) * sin(theta)
            mgcostheta = (gravity[1] + newTransformVector[1]) * cos(theta)
            
            normalVector = [-1* mgcostheta * cos(theta + pi/2), -1* mgcostheta * sin(theta + pi/2)]
            ...
            totalNormalVector = addTuplesAsVectors([totalNormalVector, normalVector])
            x, y = getIntersectionPoint((nextPoint[0],nextPoint[1],positionLibrary[point][0],positionLibrary[point][1]),(liq))

        #print(totalNormalVector)
            positionTuple = [x, y]
            #velocityTuple = [0, -10]

    velocityTuple[0] += gravity[0] + newTransformVector[0] + totalNormalVector[0]
    velocityTuple[1] += gravity[1] + newTransformVector[1] + totalNormalVector[1]

    positionTuple[0] += velocityTuple[0]
    positionTuple[1] += velocityTuple[1]
